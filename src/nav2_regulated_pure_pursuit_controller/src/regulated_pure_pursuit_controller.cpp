  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  if (shouldRotateToGoalHeading(carrot_pose)) {
    ...
    const double path_cost = heuristics::costAlongPath(
      pose_cost, pose.pose, transformed_plan, carrot_pose.pose.position, costmap);
    applyConstraints(
      regulation_curvature, speed, path_cost, transformed_plan,
