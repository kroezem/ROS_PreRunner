...
{
public:
  PlanningCostmapRestoreGuard(
    GridCollisionChecker & collision_checker,
    AStarAlgorithm<NodeHybrid> & a_star,
    nav2_costmap_2d::Costmap2D * live_costmap)
  : collision_checker_(collision_checker),
    a_star_(a_star),
    live_costmap_(live_costmap) {}

  ~PlanningCostmapRestoreGuard()
  {
    collision_checker_.setCostmap(live_costmap_);
    a_star_.setCollisionChecker(&collision_checker_);
    NodeHybrid::setObstacleHeuristicCostmap(live_costmap_);
  }

private:
  GridCollisionChecker & collision_checker_;
  AStarAlgorithm<NodeHybrid> & a_star_;
  nav2_costmap_2d::Costmap2D * live_costmap_;
};
  ...
  std::function<bool()> cancel_checker)
{
  ...
  // Copy its complete grid and geometry coherently under the live mutex, then
  // plan exclusively against the request-owned immutable snapshot.
  std::unique_ptr<nav2_costmap_2d::Costmap2D> costmap_snapshot;
  {
    std::unique_lock<nav2_costmap_2d::Costmap2D::mutex_t> lock(*(_costmap->getMutex()));
    costmap_snapshot = std::make_unique<nav2_costmap_2d::Costmap2D>(*_costmap);

    // Preserve the existing per-request footprint refresh while the planning
  ...
  // downsampled derivative). Restore member pointers before request-owned
  // storage is destroyed, including every exception and early return.
  _collision_checker.setCostmap(costmap);
  _a_star->setCollisionChecker(&_collision_checker);
  NodeHybrid::setObstacleHeuristicCostmap(costmap);
  PlanningCostmapRestoreGuard restore_costmap(
    _collision_checker, *_a_star, _costmap);
