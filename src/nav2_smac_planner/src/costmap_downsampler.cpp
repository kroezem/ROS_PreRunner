...
nav2_costmap_2d::Costmap2D * CostmapDownsampler::downsample(
  const unsigned int & downsampling_factor)
{
  return downsample(*_costmap, downsampling_factor);
}
...
void CostmapDownsampler::updateCostmapSize(
  const nav2_costmap_2d::Costmap2D * costmap)
{
  const auto * source = costmap ? costmap : _costmap;
  _size_x = source->getSizeInCellsX();
  _size_y = source->getSizeInCellsY();
  ...
  _downsampled_resolution = _downsampling_factor * source->getResolution();
}
  ...
  for (unsigned int i = 0; i < _downsampling_factor; ++i) {
    ...
    for (unsigned int j = 0; j < _downsampling_factor; ++j) {
      ...
      cost = _use_min_cost_neighbor ?
        std::min(cost, source->getCost(mx, my)) :
        std::max(cost, source->getCost(mx, my));
    }
  }
