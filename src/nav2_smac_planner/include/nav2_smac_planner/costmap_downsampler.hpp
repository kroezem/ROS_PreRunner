...
public:
  ...
  /**
   * @brief Downsample an explicit request-owned source without retaining it
   * @param costmap Immutable source costmap for this downsampling operation
   * @param downsampling_factor Multiplier for the costmap resolution
   * @return A ptr to the downsampled costmap owned by this downsampler
   */
   ...
   */
  void resizeCostmap(const nav2_costmap_2d::Costmap2D * costmap = nullptr);

protected:
  /**
   * @brief Update the sizes X-Y of the costmap and its downsampled version
   */
  void updateCostmapSize(const nav2_costmap_2d::Costmap2D * costmap = nullptr);
