...
TEST(NodeHybridTest, test_obstacle_heuristic)
{
  ...
  *costmap = *costmapA;
  ...
  nav2_smac_planner::NodeHybrid::resetObstacleHeuristic(
    costmap_ros->getCostmap(), testA.pose.x, testA.pose.y, testB.pose.x, testB.pose.y);
  ...
  // The obstacle heuristic must retain the explicitly selected request grid.
  // Mutating the live grid after taking this copy cannot change lazy heuristic
  // expansion that occurs later during the same search.
  nav2_costmap_2d::Costmap2D snapshot(*costmap);
  nav2_smac_planner::NodeHybrid::resetObstacleHeuristic(
    &snapshot, testA.pose.x, testA.pose.y, testB.pose.x, testB.pose.y);
  for (unsigned int i = 45; i <= 55; ++i) {
    for (unsigned int j = 0; j < 40; ++j) {
      costmap->setCost(i, j, 254);
    }
  }
  float snapshot_cost = nav2_smac_planner::NodeHybrid::getObstacleHeuristic(
    testA.pose,
    testB.pose,
    info.cost_penalty);
  EXPECT_EQ(snapshot_cost, wide_passage_cost);

  nav2_smac_planner::NodeHybrid::resetObstacleHeuristic(
    costmap_ros->getCostmap(), testA.pose.x, testA.pose.y, testB.pose.x, testB.pose.y);
  float disconnected_live_cost = nav2_smac_planner::NodeHybrid::getObstacleHeuristic(
    testA.pose,
    testB.pose,
    info.cost_penalty);
  EXPECT_NE(disconnected_live_cost, snapshot_cost);
